...
it('can handle login', (done) => {
  ...
  mock.onPost('/api/users/login', mockUserCredentials).reply(HTTP_OK, mockLoginResponse);
  const wrapper = shallow<AppState>(<App />);
  wrapper.find('input[placeholder=\'email\']').simulate('change', {
    target: {
      value: mockUserCredentials.email
    }
  });
  wrapper.find('input[placeholder=\'password\']').simulate('change', {
    target: {
      value: mockUserCredentials.password
    }
  });
...
it('can catch login errors', (done) => {
  ...
  mock.onPost('/api/users/login').reply(HTTP_BAD_REQUEST);
  const wrapper = shallow<AppState>(<App />);
  wrapper.setState({
    email: 'user-email',
    password: 'user-password'
...
it('can get data', (done) => {
  ...
  mock.onGet('/api/items').reply(HTTP_OK, mockItemsResponse);
  const wrapper = shallow<AppState>(<App />);
  wrapper.setState({
    isLoggedIn: true
  });
...
it('can catch data errors', (done) => {
  ...
  mock.onGet('/api/items').reply(HTTP_BAD_REQUEST);
  const wrapper = shallow<AppState>(<App />);
  wrapper.setState({
    isLoggedIn: true
  });
