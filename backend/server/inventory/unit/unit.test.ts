...
describe('/api/inventory/unit tests', () => {
  ...
  it('Should list units', async() => {
    ...
    const response3 = await request(app).get('/api/inventory/unit')
      .set('Authorization', `Bearer ${serverToken}`);
    expect(response3.status).toBe(HTTP_OK);
    const units: string[] = response3.body;
    const unitsLength = 2;
    expect(units.length).toBe(unitsLength);
    expect(units[0]).toBe('FBO One');
    expect(units[1]).toBe('FBO Two');

  });


  it('Should list only distinct units', async() => {
  ...
  it('Should list empty if no units', async() => {
