...
describe("/api/items tests", () => {
  ...
  it("should post items", async () => {
    const response = await request(app).post("/api/items").set("Authorization", `Bearer ${token}`).send({ name: "new item", value: 2000 });
    expect(response.status).toBe(200);
    expect(response.body).toBe("Item saved!");
  });

  it("should catch errors when posting items", async () => {
    const response = await request(app).post("/api/items").set("Authorization", `Bearer ${token}`).send({});
